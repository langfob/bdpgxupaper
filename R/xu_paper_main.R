...
if (bdprob@prob_is_ok)
    ...
    reloaded_bdprob = readRDS (saved_bdprob_filename)
        ...
        #  When I clicked on the saved_bdprob.rds file in the Finder or in
        #  RStudio to view it, it generated the following in the console. It
        #  turns out that the error is due to a flaw in the Finder and RStudio
        #  not recognizing the .rds extension as something that needs to be
        #  reloaded using readRDS() instead of load().
        #  You can see this because I didn't type the load() command at the
        #  console.  That was generated by the Finder/RStudio.
        #  So, even though it seems like there's an error in the saving of
        #  the data structure when you try to check it, it's fine.

# > load("/Users/bill/D/Projects/ProblemDifficulty/pkgs/bdpgxupaper/saved_bdprob.rds")
# Error: bad restore file magic number (file may be corrupted) -- no data loaded
# In addition: Warning message:
# file ‘saved_bdprob.rds’ has magic number 'X'
#   Use of save versions prior to 2 is deprecated

        #  Tried another way (and it worked, but it forces the reloaded
        #  variable to be called bdprob instead of allowing it to be
        #  assigned to any other name if you want).
##    save (bdprob, file=saved_bdprob_filename)
#    rm (bdprob)
##    load (saved_bdprob_filename)
#    browser()
    } else
    {
    cat ("\n\n>>>>>  gen_bdprob() failed.  <<<<<\n\n")
